...
from gpsocio import GPSocioModel, GPSocioForSeqRec, GPSocioTokenizer, GPSocioConfig
from collator import FinetuneDataCollatorWithPadding, EvalDataCollatorWithPadding
from dataloader import GPSocioTrainDataset, GPSocioEvalDataset
...
tokenizer_glb: GPSocioTokenizer = None
...
def encode_all_items(model: GPSocioModel, tokenizer: GPSocioTokenizer, tokenized_items, args):
...
def eval(model, dataloader, args, seq_keys, path_asser_embedding):
    ...
    for batch, labels in tqdm(dataloader, ncols=100, desc='Evaluate'):
        ...
        with torch.no_grad():
            scores,pooler_output = model(**batch)
            # print("pooler_output",pooler_output.shape)
            seq_emb[seq_keys[idx]] = pooler_output[0].tolist()
    ...
    # with open(path_tokenized_seq, "w") as f:
    with open(path_asser_embedding, "w") as f:
        json.dump(seq_emb, f)
...
def main():
    ...
    config = GPSocioConfig.from_pretrained(args.model_name_or_path)
    ...
    tokenizer = GPSocioTokenizer.from_pretrained(args.model_name_or_path, config)
    ...
    path_tokenized_seq = dir_preprocess / f'tokenized_sequence_{path_corpus.name}.json'
    ...
    train_data = GPSocioTrainDataset(train, collator=finetune_data_collator)
    val_data = GPSocioEvalDataset(train, val, test, mode='val', collator=eval_data_collator)
    test_data = GPSocioEvalDataset(train, val, test, mode='test', collator=eval_data_collator)
    ...
    path_asser_embedding = dir_preprocess / f'asser_embeddings_{path_corpus.name}.json'
    ...
    seq_keys = list(train.keys())
    test_metrics = eval(model, test_loader, args, seq_keys,path_asser_embedding)
