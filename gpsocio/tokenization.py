...
class GPSocioTokenizer(LongformerTokenizer):
    @classmethod
    ...
    def encode(self, items, encode_item=True):
        ...
        for item_idx, item in enumerate(items):
            ...
            if item_idx != len(items) -1 :
                influence_position_ids += [1] * len(item_input_ids)
            else:
                influence_position_ids += [2] * len(item_input_ids)

        input_ids = input_ids[:self.config.max_token_num]
        item_position_ids = item_position_ids[:self.config.max_token_num]
        influence_position_ids = influence_position_ids[:self.config.max_token_num]
    ...
    def padding(self, item_batch, pad_to_max):
        ...
        if pad_to_max:
            ...
            max_length = max([len(items["input_ids"]) for items in item_batch], default=10)
        

        batch_input_ids = []
        batch_item_position_ids = []
        batch_influence_position_ids = []
        ...
        for items in item_batch:
            ...
            influence_position_ids = items["influence_position_ids"]
            ...
            influence_position_ids += [self.config.max_item_embeddings - 1] * length_to_pad
            ...
            batch_influence_position_ids.append(influence_position_ids)
        ...
        return {
            ...
            "influence_position_ids": batch_influence_position_ids,
...
if __name__ == "__main__":
    ...
    from models import GPSocioConfig


    config = GPSocioConfig.from_pretrained("allenai/longformer-base-4096")
    tokenizer = GPSocioTokenizer.from_pretrained("allenai/longformer-base-4096", config=config)
