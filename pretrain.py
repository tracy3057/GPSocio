...
from gpsocio import GPSocioForPretraining, GPSocioTokenizer, GPSocioConfig, LitWrapper
...
parser.add_argument('--valid_step', type=int, default=5000)
...
def main():
    ...
    train_source_data = json.load(open(args.train_file))
    # print(train_source_data[0:10])
    dev_source_data = json.load(open(args.dev_file))
    data_collator = PretrainDataCollatorWithPadding(tokenizer, tokenized_items, mlm_probability=args.mlm_probability)
    train_data = ClickDataset(train_source_data, data_collator)
    dev_data = ClickDataset(dev_source_data, data_collator)
    ...
    pytorch_model = GPSocioForPretraining(config)
