...
from gpsocio import GPSocioTokenizer
import torch
import unicodedata
import random
...
class PretrainDataCollatorWithPadding:
    ...
    def sample_pairs(self, batch_item_ids):
        ...
        batch_item_seq_a = []
        batch_item_seq_b = []
        # print("batch_item_ids",len(batch_item_ids))
        for item_ids in batch_item_ids:
            ...
            start = max(3,start)
            sample_cnt = min(item_seq_len-1-start, 2)
            if sample_cnt <= 0:
                continue
            target_pos_list = random.sample(range(start, item_seq_len-1), sample_cnt)
            # target_pos = random.randint()
            for target_pos in target_pos_list:
                batch_item_seq_a.append(item_ids[:target_pos])
                batch_item_seq_b.append([item_ids[target_pos]])

        # print("batch_item_seq_abatch_item_seq_a",batch_item_seq_a)
        # print("batch_item_seq_bbatch_item_seq_b",batch_item_seq_b)
